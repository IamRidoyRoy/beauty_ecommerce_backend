...
class AdminReturnViewSet(ReadOnlyModelViewSet):
    permission_classes=[ReturnAdmin]; serializer_class=ReturnRequestSerializer; queryset=ReturnRequest.objects.select_related("order","user","reviewed_by").prefetch_related("items__order_item").order_by("-id"); filterset_fields=("status",)
...
class AdminRefundViewSet(ReadOnlyModelViewSet):
    permission_classes=[Finance]; serializer_class=RefundSerializer; queryset=Refund.objects.select_related("order","payment","created_by").order_by("-id"); filterset_fields=("status",)
