...
class Command(BaseCommand):
    ...
    def handle(self,*args,**opts):
        ...
        for idx in range(4):
            ...
            order=result["order"]
            for status in [Order.Status.CONFIRMED,Order.Status.PROCESSING,Order.Status.PACKED,Order.Status.SHIPPED,Order.Status.OUT_FOR_DELIVERY,Order.Status.DELIVERED]: order=transition_order(order=order,new_status=status,actor=admin)
