...
class ReviewTests(TestCase):
    def test_verified_purchase_is_server_derived(self):
        user=User.objects.create_user(phone="01766666666",password="x",full_name="Customer"); p,_,_=simple_product(); cart=Cart.objects.create(user=user); add_cart_item(cart=cart,product=p,quantity=1); ship=ShippingMethod.objects.create(name="S",code="s",base_charge=0); order=checkout(cart=cart,customer_data={"name":"Customer","phone":user.phone,"district":"Dhaka","thana":"Dhanmondi","address":"A","label":""},shipping_method=ship,payment_method="cod",request_user=user)["order"]
