...
class ReturnTests(TestCase):
    def test_return_quantity_and_restock(self):
        ...
        for status in [Order.Status.CONFIRMED,Order.Status.PROCESSING,Order.Status.PACKED,Order.Status.SHIPPED,Order.Status.OUT_FOR_DELIVERY,Order.Status.DELIVERED]: order=transition_order(order=order,new_status=status)
