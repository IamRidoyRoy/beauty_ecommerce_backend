...
class OrderLifecycleTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(phone="01744444444", password="x", full_name="Customer")
        self.ship = ShippingMethod.objects.create(name="Ship", code="ship", base_charge=Decimal("0"))
        self.city, self.thana, _ = delivery_location()
        self.p, self.si, self.wh = simple_product(stock=5)

    def place(self):
        cart = Cart.objects.create(user=self.user)
        add_cart_item(cart=cart, product=self.p, quantity=2)
        return checkout(
            cart=cart,
            customer_data={"name": "Customer", "phone": self.user.phone, "district": self.city, "thana": self.thana, "address": "Test", "label": "Home"},
    ...
    def test_place_reserves_and_cancel_releases(self):
        ...
        order = transition_order(order=order, new_status=Order.Status.CANCELLED)
        stock.refresh_from_db()
        self.assertEqual((stock.available_stock, stock.reserved_stock), (5, 0))
