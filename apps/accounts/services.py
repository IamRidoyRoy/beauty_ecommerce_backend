...
def create_otp(phone, purpose=OTPChallenge.Purpose.LOGIN):
    ...
    OTPChallenge.objects.create(phone=phone, purpose=purpose, code_hash=_hash(code), expires_at=timezone.now()+timedelta(minutes=5))
...
def verify_otp(phone, code, purpose=OTPChallenge.Purpose.LOGIN):
    ...
    if challenge.attempts > 5 or challenge.code_hash != _hash(code):
